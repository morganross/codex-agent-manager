...
## Local CAM docs bundle

CAM usage docs must be available locally on the machine where CAM is installed.

On every installed desktop or remote CAM node, bootstrap installs a local docs bundle under:
...
That bundle includes the local CAM `README.md`, `howto-use-qexow-cam.md`, and `qexow-cam-plain-english.md`. Generated skills should point to these local paths first instead of sending agents to unrelated remote repos.

## Command path rules

Do not assume one CAM command shape works everywhere.

Use the command form that matches your current machine:

- local Windows repo checkout: `.\cam.cmd ...`
- local Windows installed CAM on `PATH`: `cam ...`
- remote Linux installed CAM on `PATH`: `cam ...`
- remote Linux legacy repo checkout at `/home/ubuntu/codex-agent-manager`: `node /home/ubuntu/codex-agent-manager/bin/cam.js ...` (legacy/dev-only)

If you are reporting current CAM state, do not quote cached output as if it were fresh. Rerun the command from the correct environment first.
...
5. Linux packaged runtime
   On remote Linux nodes, the preferred steady-state shape is an installed package with a bundled private Node runtime, installed `cam` launcher, local docs bundle, and a user-level `systemd` service.
...
If you are not on local Windows in this repo checkout, translate these examples to the matching command form from the command-path rules above.

## Linux package behavior

Linux remote CAM should no longer depend on a live repo checkout plus a system Node install.

The preferred Linux install shape is:

- package-owned app root under `/opt/qexow-cam`
- stable launcher at `/usr/local/bin/cam`
- bundled private Node runtime under the app root
- local docs bundle under `~/.qexow-cam/docs/`
- user-level `systemd` service for the installed operator user

Default package behavior is intentionally aggressive:

- reinstall stops old CAM services and processes
- reinstall wipes CAM runtime state unless preserve-state is explicitly requested
- uninstall removes CAM runtime state, launchers, and service definitions

The legacy repo path `/home/ubuntu/codex-agent-manager` may still exist during migration, but it is not the preferred steady-state runtime path.
...
Remote Linux installed equivalent:

```bash
cam agent list
cam send backend-local "Please reply with your node name and cwd."
```

Remote Linux legacy repo-checkout equivalent:

```bash
node /home/ubuntu/codex-agent-manager/bin/cam.js agent list
node /home/ubuntu/codex-agent-manager/bin/cam.js send backend-local "Please reply with your node name and cwd."
```
...
- local Windows repo checkout: `.\cam.cmd inventory export`
- remote Linux installed CAM: `cam inventory export`
- remote Linux legacy repo checkout: `node /home/ubuntu/codex-agent-manager/bin/cam.js inventory export`

If the remote node is on an older CAM install, CAM can fall back to:

- local Windows repo checkout: `.\cam.cmd daemon status` and `.\cam.cmd agent list`
- remote Linux installed CAM: `cam daemon status` and `cam agent list`
- remote Linux legacy repo checkout: `node /home/ubuntu/codex-agent-manager/bin/cam.js daemon status` and `node /home/ubuntu/codex-agent-manager/bin/cam.js agent list`
...
- `npm run build:linux-package -- --arch <x64|arm64> --node-bin <path>` builds a Linux `.deb` package with a bundled runtime
- the release workflow compiles the installer and runs installer smoke checks
- the release workflow also builds Linux `amd64` and `arm64` `.deb` packages
...
- old remote nodes may not support `inventory export`, so CAM still needs the older `daemon status + agent list` path, but those commands still have to be run with the correct machine-specific wrapper
- some remote Linux nodes may remain repo-based during migration, but packaged `/usr/local/bin/cam` is the intended steady-state command surface
